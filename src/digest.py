"""Durable Moderate delivery and daily New York-time health digests."""
...
def run_moderate_digest(state, notifier, now, *, policy=None, message_limit=3900):
    immediate = getattr(policy, 'moderate_immediate', False)
    options = {} if policy is None else {'timezone_name': policy.timezone, 'send_after': policy.send_after}
    day = None if immediate else digest_due(now, state.state['digest'].get('last_processed_date'), **options)
    if day is None and not immediate:
    ...
    try:
        ...
        report = None
        if due_ids:
            heading = 'Verified job matches' if immediate else 'Daily moderate matches'
            report = deliver_pending(heading=heading, queue_kind='moderate', notifier=notifier,
