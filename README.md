...
Deployment status: Telegram live alerts are enabled and real job delivery is verified. The September 8 repair delivered nine current job listings in five Telegram messages and saved all delivery receipts. See [deployment status and verification evidence](docs/deployment-status.md).
...
- **Moderate (55–74)**: sent after each successful scan, without waiting until evening. Already queued Moderate matches are included, and recorded deliveries are not intentionally resent.
- **Skip (below 55 or any hard eligibility failure)**: no alert.
- No new qualifying role means no routine message. Persistent problems with high-priority sources get a separate daily health summary after **7:30 PM America/New_York**.

The production configuration enables `digest.moderate_immediate: true`. Setting it to `false` restores the first-successful-run-after-7:30-PM Moderate digest. This timing switch does not change job eligibility, scoring, CV selection, baselines, or delivery history. With immediate mode enabled, a later qualifying match can send on another scan the same day.
...
To receive useful jobs immediately after setup, manually enable **current_roundup** with a dry-run preview, then a live scan. This freshly verifies the official listings and queues at most **10 unsent current matches**, ranked by fit, for immediate delivery, including eligible Moderate matches even when daily Moderate delivery is configured. A seeded listing with no published posting date may qualify for this explicit roundup only, with no recency bonus and no claim that it is newly posted. Known old posting dates, incompatible authorization, unresolved employment, and excessive required experience are still withheld. Existing delivery history is preserved. The option defaults to false and is never enabled by the automatic schedule. Selecting it again may send another batch of still-unsent matches, not resend recorded deliveries.

Every alert includes the company, exact title, job ID when available, location/work arrangement, posting date or first seen, salary, experience, employment status, sponsorship caveat, fit score, main gap, recommended CV, and official application link. Messages use plain-language labels; detailed source provenance stays in the internal alert record. Fit scores and CV choices are tracker assessments, not employer endorsements. Required years are summarized without pasting an unfinished qualifications paragraph. Unknown or ambiguous information remains explicitly uncertain.

Degree matching respects explicit alternatives. A published allowance for another technical or engineering degree can match the candidate's documented engineering degree. A related-field or equivalent-experience alternative that cannot be confirmed is flagged for review, not asserted as a definite mismatch or awarded automatic qualification credit. Explicit degree requirements still produce a gap when unsupported by the profile.

Sponsorship that is not explicitly supported by the posting remains uncertain. **Visa sponsorship does not imply green-card support.** Confirm that separately with the employer. Presentation updates apply to future messages and do not edit existing Telegram messages or reset delivery history.
